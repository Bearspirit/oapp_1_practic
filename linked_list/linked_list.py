...
# Для установки курсора в конец списка можно вызвать команду head и затем вызывать команду right пока не дойдём до конца.
# Этот подход имеет сложность O(N).
# В то же время, при эффективной реализации списка (хранится ссылка на конец) сложность команды tail будет O(1).
