...
class HashTable:
    ...
    def __init__(self, capacity: int, min_capacity: Optional[int] = None,
        ...
        self._buffer = HashBuffer(size, self.__max_collisions)
        self.__put_status = self.PutStatus.NIL
        self.__delete_status = self.DeleteStatus.NIL
    ...
    def delete(self, value: Any) -> None:
        index = self._buffer.find_cell(value)
        if self._buffer.get_find_cell_status() != \
    ...
    def put(self, value: Any) -> None:
        ...
        if self._buffer.get_find_cell_status() == \
            ...
            self._buffer.put(index, value)
            assert(self._buffer.get_put_status() == Buffer.PutStatus.OK)
            self.__put_status = self.PutStatus.OK
            return
        if self._buffer.get_find_cell_status() == \
                HashBuffer.FindCellStatus.VALUE_FOUND:
            ...
            return
        assert(self._buffer.get_find_cell_status() == \
    ...
    def contains(self, value: Any) -> bool:
        self._buffer.find_cell(value)
        return self._buffer.get_find_cell_status() == \
            HashBuffer.FindCellStatus.VALUE_FOUND
    ...
    def __reset_buffer(self) -> None:
        while True:
            ...
            if _try_copy_hash_buffer(self._buffer, new_buffer):
